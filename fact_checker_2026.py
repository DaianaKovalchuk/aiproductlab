...
def fact_check_sentences(
    ...
    for s in candidates:
        ...
        if not wiki_candidates and not web_candidates:
            results.append(
                FactCheckResult(
                    ...
                    explanation="Подходящие статьи в открытых источниках не найдены.",
                )
            )
            continue
        ...
        if best_sim >= 0.7 and not numbers_conflict:
            status = "confirmed"
            explanation = "Смысл предложения хорошо совпадает с источниками."
        elif best_sim >= 0.55 and not numbers_conflict:
            status = "partial"
            explanation = "Источники описывают похожий факт, но есть отличия."
        elif best_sim <= 0.35 or numbers_conflict:
            status = "contradicted"
            if numbers_conflict:
                explanation = "Числа/даты в предложении отличаются от источников."
            else:
                explanation = "Описание в источниках отличается по смыслу."
        else:
            status = "no_source"
            explanation = "Не удалось найти подтверждение."
